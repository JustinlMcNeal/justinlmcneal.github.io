...
export function renderProductCard(product) {
  ...
  const dotsHtml = shown
    .map((c) => {
      const { background, isMultiColor } = parseColorValue(c);
      ...
      return `<span class="inline-block w-3 h-3 rounded-full border border-black/15 ${borderClass}" title="${escapeHtml(c)}" style="background: ${background}"></span>`;
    })
    .join("");
