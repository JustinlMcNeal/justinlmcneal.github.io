-- Clearer approve_parcel_import_cpi error when USD CPI cannot be derived.
...
BEGIN
  ...
  v_has_fx := coalesce(v_import.effective_fx_rate, 0) > 0
    OR EXISTS (
      SELECT 1
      FROM public.parcel_import_cost_allocations a
      WHERE a.parcel_import_id = p_import_id
        AND a.allocation_run_type = 'preview'
        AND coalesce(a.effective_fx_rate, 0) > 0
    );

  IF NOT v_has_fx THEN
    RAISE EXCEPTION 'Effective exchange rate required — set FX rate on parcel charges and save draft'
      USING ERRCODE = 'P0001';
  END IF;
  ...
  IF v_variants_updated + v_products_updated = 0 THEN
    RAISE EXCEPTION 'No product CPI targets updated — matched business rows need positive quantity and landed cost'
      USING ERRCODE = 'P0001';
  END IF;
