...
export async function calculateCartTotals(items) {
  ...
  // 2) Auto %/fixed promos - pass items for per-item fixed discount calculation
  const { totalDiscount: autoPromoDiscount, breakdown: promoBreakdown } =
    calculatePromotionDiscount(autoPromos, subtotal, safeItems);
  ...
  return {
    ...
    // ✅ expose active promos for recommendations
    activePromos: autoPromos,
  };
}
