-- Phase 10M — Marketplace refund/cancel integration into return guidance views.
...
SELECT
  ...
  CASE
    ...
    WHEN refund_guidance_status IN ('marketplace_refund_review', 'cancellation_detected', 'return_detected')
      ...
      'Marketplace refund/cancel signal detected. Confirm physical return before restocking.'
...
COMMENT ON VIEW public.v_inventory_bundle_component_return_guidance IS
  'Phase 10M: Stripe + marketplace refund/cancel observability + suggested restock guidance (no auto-restock).';
...
COMMENT ON VIEW public.v_inventory_bundle_component_return_workflow_guidance IS
  'Phase 10M: return guidance + RMA workflow + Stripe/marketplace refund guidance overlay.';
