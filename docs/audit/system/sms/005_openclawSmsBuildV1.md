...
Source of truth: `docs/audit/system/sms/003_smsAudit.md` and `docs/audit/system/sms/004_openclawSmsPlan.md`  
...
| Item | Why excluded |
|---|---|
| Autonomous or scheduled SMS sending | Not safe yet per `004_openclawSmsPlan.md` §7 |
...
| Raw customer targeting or phone list generation | PII restriction per `004_openclawSmsPlan.md` §9 |
...
(List any warnings about the data itself. Examples: a view returned zero rows, a field was missing or null, date filtering was not possible, a known audit gap may affect this data. Cite specific audit findings from 003_smsAudit.md where relevant.)
...
docs/
  ...
  audit/
    system/
      003_smsAudit.md            ← Existing audit (source of truth)
      004_openclawSmsPlan.md     ← Existing plan (source of truth)
      005_openclawSmsBuildV1.md  ← This file
```
...
The `004_openclawSmsPlan.md` §9 explicitly requires:
...
1. A new Supabase migration that creates the `sms_drafts` table using the schema defined in `004_openclawSmsPlan.md` §5.
...
V3 is the admin approval UI. V3 lets a human see all pending drafts, edit copy, and mark a draft `approved` or `rejected`. V3 is Phase 3 from `004_openclawSmsPlan.md`. It requires the fixes listed in §11 of that plan before any approved draft is ever used in an actual send.
