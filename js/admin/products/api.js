 ...
 * @param {string} folder - Subfolder path (e.g., "catalog", "gallery", "variants")
 * @returns {Promise<string>} Public URL of the uploaded image
 */
export async function uploadProductImage(file, folder = "images") {
  const client = sb();
 ...
 */
async function ensureProductBucket(client) {
  try {
    // Check if bucket exists by trying to list it
    const { data: buckets, error: listError } = await client.storage.listBuckets();
    
    if (listError) {
      console.warn('Could not list buckets (may be RLS restriction):', listError.message);
    ...
    if (!exists) {
      console.warn('Products bucket not found. Please create it manually in Supabase Dashboard:');
      console.warn('1. Go to Storage in your Supabase project');
      console.warn('2. Click "New Bucket"');
      console.warn('3. Name it "products" and enable "Public bucket"');
      // Don't throw - let the upload attempt proceed and fail with a clearer error
    }
  } catch (err) {
    // If we can't check, just proceed - the upload will fail with a clear error if bucket doesn't exist
