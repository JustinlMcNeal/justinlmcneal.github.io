-- Phase 10M — Multi-channel refund/cancel observability (read-only view).
...
FROM (
  ...
  SELECT
    ...
    NULL::integer AS quantity_refunded,
    NULL::integer AS quantity_returned,
  ...
  -- Marketplace order-level refund fields on orders_raw
  SELECT
  ...
  -- Fulfillment cancellation / carrier return signals
  SELECT
    ...
    CASE WHEN fs.label_status = 'cancelled' THEN 'cancelled' ELSE NULL END AS cancellation_status,
    ...
    'order_sync'::text AS sync_source,
    CASE
      WHEN fs.label_status = 'cancelled' THEN 'cancellation'
  ...
  -- Amazon finance refund/return/chargeback transactions (when synced)
  SELECT
    ...
    NULL::text AS cancellation_status,
    CASE
      WHEN aft.transaction_type ILIKE '%return%' THEN 'returned'
      ELSE NULL
    ...
    'order_sync'::text AS sync_source,
    CASE
      WHEN aft.transaction_type ILIKE '%refund%'
  ...
  -- eBay finance REFUND/CREDIT rows (when present)
  SELECT
  ...
  WHERE eft.stripe_checkout_session_id LIKE 'ebay%'
    AND UPPER(COALESCE(eft.transaction_type, '')) IN ('REFUND', 'CREDIT')
) AS observations
WHERE source_order_id IS NOT NULL;

COMMENT ON VIEW public.v_inventory_marketplace_refund_observations IS
  'Phase 10M: read-only unified refund/cancel/return signals from Stripe cache, orders_raw, fulfillment, and marketplace finance sync.';
