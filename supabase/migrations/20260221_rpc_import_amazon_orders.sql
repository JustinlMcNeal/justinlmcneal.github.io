-- Migration: Create rpc_import_amazon_orders function
...
BEGIN
  ...
  -- 3) Upsert fulfillment_shipments (mark Amazon orders as shipped)
  ...
  SELECT
    ...
    (o->>'label_cost_cents')::int,
    'Imported from Amazon Seller Central'
  FROM jsonb_array_elements(p_orders) AS o
  ON CONFLICT (stripe_checkout_session_id) DO UPDATE SET
    label_status     = EXCLUDED.label_status,
    carrier          = COALESCE(fulfillment_shipments.carrier, EXCLUDED.carrier),
    service          = COALESCE(fulfillment_shipments.service, EXCLUDED.service),
...
-- Grant execute to authenticated users (admin guard already protects the page)
