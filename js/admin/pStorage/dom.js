export function getEls() {
  return {
    ...
    profitProjectionsPanel: document.getElementById("profitProjectionsPanel"),
  };
}

export function setStatus(els, msg) {
  if (els?.statusEl) {
    const isLoaded = msg.toLowerCase().includes("loaded");
    const isError = msg.toLowerCase().includes("error");
    const dotColor = isError ? "bg-red-500" : (isLoaded ? "bg-green-500" : "bg-gray-400 animate-pulse");
